...
状态：`CONTROL_TEST_BANK_VERIFIED_DATA_CHANNEL_DESIGN`
...
当前 CONTROL V1、静默 watchdog 和测试寄存器 bank 已完成仿真、构建及板级验证。
产品寄存器接入暂缓，先设计 DATA 通道、差异化缓存、双端口隔离、队列提交完成接口和 Jumbo Frames 支持。
本文件维护模块之间的集成提案；DATA 内部设计归属独立模块文档，尚未进入 RTL 实现。
...
| Ethernet 层 | TEMAC/PCS-PMA、FCS、MAC client FIFO、三速时钟边界 | 业务协议 |
...
“已有 UDP”特指本项目 `udp_top/udp_transport_fixed_host`，不是原工程 `lan_tx`。
当前只有端口 32000 和一组完整 payload RX/TX 接口，全部接给 CONTROL；标准 MTU 1500，
最大 payload 1472 Byte，RX 4 槽、TX 2 槽同步 BRAM。当前仍使用这套已验收 RTL 和镜像。

本阶段设计扩展为 `ctrl_rx/ctrl_tx` 与 `data_rx/data_tx` 两组逻辑接口。CONTROL 保持端口 32000，
DATA 的本地/主机端口设计默认均为 32001，可综合期配置，须与 CONTROL 区分；默认继续固定唯一主机，不引入运行时端口协商。
CONTROL 的线上 16-Byte 格式、W=4 及寄存器接口不因 DATA 改变。

Jumbo Frames 是已确认的新需求；至少兼容默认 8172-byte payload，对应 IP MTU 至少 8200。
当前主机已核对 JumboPacket=9KB、IPv4 MTU=9000；首版设计上限为 MTU9000/payload8972，FPGA 支持能力仍待验证。
现有硬件性能记录仍仅证明标准 MTU 基线，不作为 Jumbo 验收。
...
CONTROL 与 DATA 各自拥有 RX/TX 队列及占用状态，并按不同内容设计：CONTROL 评估紧凑的
16-Byte 小记录 FIFO；DATA 已选择同步 BRAM 字节环+描述符 FIFO，RX/TX 各 32 KiB、各 16 条描述符。
...
CONTROL 采用有连续服务上限的优先仲裁；存在其他待发帧时，首版建议连续最多 4 帧 CONTROL，
...
Jumbo、正常 FIFO 积压、三速链路与 CONTROL 超时共同验证；只有发现明确延迟目标未满足时再评估调整。
...
CONTROL 软复位只使旧 CONTROL 事务和未发送队列失效；已被共享引擎选中的 CONTROL 帧必须保有
独立且稳定的 payload 所有权直到收尾，不能清零它仍在读取的槽。已进入 Ethernet FIFO 的旧 CONTROL
帧可排空，不对它新增业务逐包网络完成反馈。RX 正在接收的旧 CONTROL 候选包在帧尾丢弃，
公共 parser 继续完成帧边界处理。具体以延迟清理或槽保留实现，不在共享帧中途强制清空状态。
...
5. 验证 DATA 满载时 CONTROL RTT、DATA 无饿死、正常 FIFO 排队/反压、三速适配和标准包兼容性。
6. 使用测试寄存器 bank 和通用 payload 发生/检查器集成，再做构建和板测；不以真实产品寄存器、
   ADC 或 DDR 已接入为前置条件。

## 8. 待收敛参数与实施顺序

DATA 的长度上限、默认端口、FIFO 组织/容量、取消方式和错误编码已形成首版设计基线，见 DATA MODULE.md
第 3～7 节。仍需收敛 CONTROL 小记录队列和仲裁连续服务上限，并验证 FPGA 的 Jumbo 支持与时序资源。

实施顺序：按已确定契约实现 DATA 缓存，收敛 CONTROL 队列和共享调度，再完成 Jumbo 路径、双通道分类与仲裁、本地提交状态
