...
set diag_top "udp_perf_diag_top"
set normal_top "udp_echo_test_top"
set synth_name "synth_udp_perf_diag"
set impl_name "impl_udp_perf_diag"
...
set_property top $diag_top [get_filesets sources_1]
update_compile_order -fileset sources_1

# The diagnostic top disables the AD9517 ILA instance while retaining the
# sequence ILA.  Suppress only the absent ILA's scoped constraints.
...
if {![string match "*Complete*" $synth_status]} {
    error "UDP diagnostic synthesis did not complete: $synth_status"
}
if {![string match "*Complete*" $impl_status]} {
    error "UDP diagnostic implementation did not complete: $impl_status"
}
...
puts "RESULT=UDP_PERF_DIAG_ROUTE_PASSED"
close_design
launch_runs $impl_name -to_step write_bitstream -jobs 4
wait_on_run $impl_name
set impl_status [get_property STATUS [get_runs $impl_name]]
if {![string match "*Complete*" $impl_status]} {
    error "UDP diagnostic bitstream generation did not complete: $impl_status"
}

open_run $impl_name
set bit_path "$project_root/led.runs/$impl_name/${diag_top}.bit"
set ltx_path "$project_root/led.runs/$impl_name/${diag_top}.ltx"
...
set_property top $normal_top [get_filesets sources_1]

puts "PROJECT=$project_path"
puts "TOP_BUILT=$diag_top"
puts "RESTORED_PROJECT_TOP=[get_property top [get_filesets sources_1]]"
...
puts "DEBUG_PROBES=[file normalize $ltx_path]"
puts "REPORT_DIR=[file normalize $report_dir]"
puts "RESULT=UDP_PERF_DIAG_BUILD_PASSED"
